...
Now try it out with the above exercise
...
We can use `lapply`, which applies a function over a list or vector and returns the output as a vector, to get output as the above for loop:
...
Repeat the prior exercise using `lapply`
```{r, echo=F, eval=F, include=F}
us_Gapminder <- filter(gapminder, continent == "Americas" & year == 2002)
