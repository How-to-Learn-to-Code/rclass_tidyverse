...
To do this we have to **map** the data onto the plot. Do do this we make an **aesthetic** (with `aes()`). **aesthetics** are parts of the plot that are **determined by the data**. Here we say that we want to plot the `Sepal.Width` on the x-axis, and `Petal.Width` on the y-axis.
...
## Exercise:
 - Let's make the same scatterplot but color the points by their `Species`. 
